...
(function(win) {
	...
	//提示弹层,弹层依赖于popDialog，popDialog依赖于zepto
	dj.iAlert = function(msg, hiddenTime) {
		if (popDialog) {
			popDialog({
				'content': msg,
				closeBtn: false,
				times: hiddenTime * 1000 || 2000,
				isLock: false,
				shadow: true
			}).show();
		} else {
	...
	var Weixin = {
		init: function() {
			...
			//隐藏微信右上角按钮
			Weixin[elHtml.getAttribute('hideMenu') === 'true' || document.getElementById('hideMenu') ? 'hideOptionMenu' : 'showOptionMenu']();
		...
		getResource: function() {
			...
			for (; i < len; i++) {
				...
				if (propertyVal && match) {
					...
					match[1] === 'title' && result[match[1]] && (document.title = result[match[1]]); //document.title ||
					/***20150104 add hack*/
				}
			}
