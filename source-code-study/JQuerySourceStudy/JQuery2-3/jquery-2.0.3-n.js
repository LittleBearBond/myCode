...
(function(window, undefined) {
		...
		init: function(selector, context, rootjQuery) {
			...
			if (typeof selector === "string") { //处理字符串
				if (selector.charAt(0) === "<" && selector.charAt(selector.length - 1) === ">" && selector.length >= 3) {
					...
					match = [null, selector, null]; //如果selector是html标签组成的话,而不用正则检查

				} else {
					match = rquickExpr.exec(selector); //是html 需要用正则表达式来检查匹配
				}
				// Match html or make sure no context is specified for #id
				if (match && (match[1] || !context)) { //匹配的html或确保没有上下文指定为# id 上面英文写的很明显了， 主要是没有直定上下文

					// HANDLE: $(html) -> $(array)
					if (match[1]) {
						context = context instanceof jQuery ? context[0] : context;

						// scripts is true for back-compat //合并两个数组内容到第一个数组
						jQuery.merge(this, jQuery.parseHTML( //使用原生的DOM元素的创建函数将字符串转换为一组DOM元素，然后，可以插入到文档中
							match[1],
				...
				// HANDLE: $(DOMElement)//处理DOMElement,返回修改过后的this,给this添加了
			...
			return jQuery.makeArray(selector, this); //用数组的方式存储
		},
	...
	/*使用时是把jQuery的原型传递给jQuery.prototype.init.prototype，这里把Query.fn.init 的原型指会给 jQuery.fn，
	 *这样可以让init，做到既能隔离作用域还能使用jQuery原型对象的作用域呢，还能在返回实例中访问jQuery的原型对象
	 */
	...
	jQuery.extend = jQuery.fn.extend = function() {
		...
		if (typeof target === "boolean") { // 如果第一个参数为true，即 jQuery.extend( true, obj1, obj2 ); 的情况
			deep = target; //此时 deep的值就是target的值
			target = arguments[1] || {}; //内部真正的target应该等于arguments[1]
		...
		if (typeof target !== "object" && !jQuery.isFunction(target)) { // 处理奇怪的情况，比如 jQuery.extend( 'hello' , {nick: 'casper})~~
			target = {}; //target不是对象也不是函数，这里直接赋值为对象
		}

		// extend jQuery itself if only one argument is passed
		if (length === i) { // 处理这种情况 jQuery.extend(obj)，或 jQuery.fn.extend( obj )，直接把对象附加到jQuery 或者jQuery.fn上
	...
	jQuery.extend({
		...
		noConflict: function(deep) {
			//之前用_$ 和_JQuery 接收了  $ 和JQuery  这里转移控制权
			if (window.$ === jQuery) {
				window.$ = _$;
			}
	...
	(function(window, undefined) {
		...
		//假设传入进来的选择器是：div > p + .bond[type="checkbox"], #id:first-child
		//这里可以分为两个规则：div > p + .bond[type="checkbox"] 以及 #id:first-child
		//返回的需要是一个Token序列
		...
		function tokenize(selector, parseOnly) {
			var matched,
				...
				tokens,
				type,
				soFar, //这里的soFar是表示目前还未分析的字符串剩余部分
				groups,
				preFilters,
			...
			preFilters = Expr.preFilter; //这里的预处理器为了对匹配到的Token适当做一些调整
			...
			while (soFar) {
				...
				if (!matched || (match = rcomma.exec(soFar))) {
					if (match) {
						...
						soFar = soFar.slice(match[0].length) || soFar; //如果匹配到逗号
					}
					groups.push(tokens = []); //往规则组里边压入一个Token序列，目前Token序列还是空的
				}
				...
				if ((match = rcombinators.exec(soFar))) {
					...
					soFar = soFar.slice(matched.length); //剩余还未分析的字符串需要减去这段已经分析过的
				}
	...
	*/
	jQuery.Callbacks = function(options) {　 //当传入参数时，就是字符串，比如：once，那么就会先去optionsCache中取once属性，如果之前没有此值，就调用createOptions('once memory')方法
		...
		var // Last fire value (for non-forgettable lists)
			...
			stack = !options.once && [], //如果有once，那么stack就是false，如果没有，那么stack就是[]，if条件里面空数组为真，因此在fire()->fireWith()时，就可以再次执行list数组中的方法了。
			// Fire callbacks
			fire = function(data) {
				memory = options.memory && data; //如果参数memory为true，则记录data，否则memory为false
				fired = true; //标记触发过回调
				firingIndex = firingStart || 0; //list数组中开始元素
				firingStart = 0;
				firingLength = list.length;
				firing = true; //正在执行list数组中的方法
				for (; list && firingIndex < firingLength; firingIndex++) {
					if (list[firingIndex].apply(data[0], data[1]) === false && options.stopOnFalse) { //函数返回false 并且设置有中断调用 那么返回false后面的就会中断调用
				...
				if (list) { //当list数组中的方法执行结束后，就会判断stack是否有值
					if (stack) { //如果堆栈存在 once为false的时候就存在
						if (stack.length) { //如果堆栈存在
							fire(stack.shift()); //从堆栈头部取出，递归fire。
						}
					} else if (memory) { //如果有once，stack就是false，就会走这里
						list = []; //清空list数组
					} else {
						self.disable(); //再否则阻止回调列表中的回调，直接清除掉后面的
					}
				}
			},
			...
			self = {
					...
					if (list) { //[] 没有被置为 undefined
						...
						(function add(args) {
							jQuery.each(args, function(_, arg) {
								...
								if (type === "function") {
									if (!options.unique || !self.has(arg)) { //unique 如果为true就需要检查是否已经添加这个函数，反之则不需要检查
										list.push(arg);
									}
						...
						if (firing) { //如果现在正处于回调触发中，这里改变firingLength 后面添加进去的就会执行
							firingLength = list.length;
							// With memory, if we're not firing then
							// we should call right away
							//第一次调用add时，memory是undefined。当调用fire时，如果你传入了memory为true，则memory就会变成true，如果没有调用过fire memory一直为undefine
							//这时你再调用add，就会进入if语句，进行fire。所以你传入memory，fire后，再调用add(b)，b方法会执行。
						} else if (memory) {
							//把firingStart 设置为之前的数组长度，这样就会执行新添加的这些回调
							firingStart = start;
							fire(memory);
						}
					}
				...
				// Is it disabled? 后面所有的操作都失效
				disabled: function() {
					return !list;
				},
				// Lock the list in its current state
				// 只会锁住后面的fire操作，让cb.fire方法失效。
				lock: function() {
					stack = undefined;
					if (!memory) {
						self.disable();
				...
				fireWith: function(context, args) { //执行list数组中的方法
					if (list && (!fired || stack)) { //list 不为undefined 没有被执行过disable操作。第一次调用时fired是undefined，第二次调用时，fired为true，就要看stack了。
						//如果执行过  disable 或者lock 这时stack为undefined 回调也不会执行
						args = args || [];
						args = [context, args.slice ? args.slice() : args];
						if (firing) { //现在正处于回调执行中
							stack.push(args);
						} else {
							fire(args);
						}
	...
	jQuery.extend({
		Deferred: function(func) {
			...
			var tuples = [
				...
				promise = {
					...
					then: function( /* fnDone, fnFail, fnProgress */ ) {
						...
						var func = function(newDefer) {
							jQuery.each(tuples, function(i, tuple) {
								var fn = jQuery.isFunction(fns[i]) && fns[i];
								// deferred[ done | fail | progress ] for forwarding actions to newDefer
								deferred[tuple[1]](function() {
									//jquery 这里支持多个参数arguments
									var returned = fn && fn.apply(this, arguments);
									//如果函数返回的是 promise对象就继续把后边的任务添加到returned。 当这个执行完毕的时候就绪继续执行之前后面的任务队列
			...
			promise.deferredNums = deferredNums;
			...
			jQuery.each(tuples, function(i, tuple) {
				...
				if (stateString) {
					//这里一次性添加了三个回调函数
					list.add(function() {
						// state = [ resolved | rejected ] 调用的时候改变当前promise的state，
						state = stateString;
						// [ reject_list | resolve_list ].disable; progress_list.lock
						// 当resolve的时候调用reject的disable； 当reject的时候调用resolve的disable
						// 执行resolve或者reject 就锁定当前回调队列
					}, tuples[i ^ 1][2].disable, tuples[2][2].lock);
				}

				// deferred[ resolve | reject | notify ]===resolveWith | rejectWith | notifyWith=== list.fireWith==(context, args)
		...
		when: function(subordinate /* , ..., subordinateN */ ) {
			...
			//因为when也要形成异步操作，比如when().done()，所以内部必须新建一个jQuery.Deferred()对象，用来给后面链式调用
