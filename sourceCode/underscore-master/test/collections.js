(function() {
	...
	test('transform', function() {
		var list = _.transform(['foo', 'bar', 'baz'], function(accumulator, value, index) {
			accumulator[index] = value + '2';
		});
		deepEqual(list, ['foo2', 'bar2', 'baz2'], 'handles arrays with no accumulator');

		list = _.transform(['foo', 'bar', 'baz'], function(accumulator, value, index) {
			accumulator[index] = value + '2';
		}, []);
		...
		deepEqual(list, {
			...
			1: 'bar2',
			2: 'baz2'
		}, 'handles arrays with object accumulator');
		...
		deepEqual(obj, {
			...
			bar: 3,
			baz: 4
		}, 'handles objects with no accumulator');
		...
		deepEqual(obj, {
			...
			bar: 3,
			baz: 4
		}, 'handles objects with array accumulator');
		...
		obj = _.transform(instance, function(accumulator, value, key) {
			accumulator[key] = value + 1;
		});
		...
		deepEqual(obj, new Obj({
			...
			bar: 3,
			baz: 4
		}), 'handles instances with no accumulator');

		var context = {},
			actualContext;
		_.transform([1], function() {
			actualContext = this;
		}, {}, context);
		...
		var accumulator = {};
		var args = [accumulator, 'foo', obj.foo, obj],
			actualArgs;
		_.transform(obj, function() {
			actualArgs = _.toArray(args);
		}, accumulator);
		...
		list = _.transform([1, 2, 3, 4], function(accumulator, value) {
			return value < 3 && accumulator.push(value);
		});
	...
	test('find', function() {
		...
		array.dontmatch = 55;
		strictEqual(_.find(array, function(x) {
			return x === 55;
	...
	test('every', function() {
		...
		ok(_.every([0, 10, 28], function(num) {
			return num % 2 === 0;
		}), 'even numbers');
		ok(!_.every([0, 11, 28], function(num) {
			return num % 2 === 0;
		}), 'an odd number');
		...
		ok(_.every(['a', 'b', 'c', 'd'], _.hasOwnProperty, {
			a: 1,
			b: 2,
			c: 3,
			d: 4
		}), 'context works');
		ok(!_.every(['a', 'b', 'c', 'd', 'f'], _.hasOwnProperty, {
			a: 1,
			b: 2,
			c: 3,
			d: 4
	...
	test('some', function() {
		...
		ok(!_.some([1, 11, 29], function(num) {
			return num % 2 === 0;
		}), 'all odd numbers');
		ok(_.some([1, 10, 29], function(num) {
			return num % 2 === 0;
		}), 'an even number');
		...
		ok(_.some(['a', 'b', 'c', 'd'], _.hasOwnProperty, {
			a: 1,
			b: 2,
			c: 3,
			d: 4
		}), 'context works');
		ok(!_.some(['x', 'y', 'z'], _.hasOwnProperty, {
			a: 1,
			b: 2,
			c: 3,
			d: 4
	...
	test('includes', function() {
		...
		ok(_.includes({
			...
			larry: 3,
			curly: 9
		}, 3) === true, '_.includes on objects checks their values');
	...
	test('where', function() {
		...
		deepEqual(_.where([_, {
			a: 1,
			b: 2
		}, _], test), [_, _], 'checks properties given function');
	});
	...
	test('findWhere', function() {
		...
		equal(_.findWhere([_, {
			a: 1,
			b: 2
		}, _], test), _, 'checks properties given function');
		...
		deepEqual(_.findWhere([{
			y: 5,
			b: 6
		}, expect], new TestClass()), expect, 'uses class instance properties');
	});
	...
	test('max', function() {
		...
		equal(_.max({
			'a': 'a'
		}), -Infinity, 'Maximum value of a non-numeric collection');
		...
		deepEqual(_.max([0, 2], function(a) {
			return a * this.x;
		}, {
			x: 1
		}), 2, 'Iterator context');
	...
	test('min', function() {
		...
		equal(_.min({
			'a': 'a'
		}), Infinity, 'Minimum value of a non-numeric collection');
		...
		deepEqual(_.min([0, 2], function(a) {
			return a * this.x;
		}, {
			x: -1
		}), 2, 'Iterator context');
	...
	test('partition', function() {
		...
		deepEqual(_.partition([1, 2, 3], predicate, {
			x: 2
		}), [
			[2],
			[1, 3]
