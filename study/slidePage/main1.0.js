/*
*author xj
* 有touchstart时间不一定有touchomve 和touchend事件，所以一些操作应该放到touchmove中去
* 判断元素少于一个的时候就不滚动
* 高度自适应由css控制
*webkitTransitionEnd 时间在安卓微信中有些问题，替换成setTimeout
*/
((function(root, factory) {
	...
	$.extend(slidePage.prototype, {
		...
		move: function(pre, next) {
			...
			/*小圆点滑动*/
			self.setTransform(self.smallPoint, disPoint);
		...
		setTransform: function(el, dis, speed) {
			...
			if (el == this.smallPoint) {
				var self = this;
				setTimeout(self.moveEnd.bind(self), self.opts.sepTime);
			}
			return el;
		}
	});
